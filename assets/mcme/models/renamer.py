...
def duplicate_and_replace(folder_path):
    for filename in os.listdir(folder_path):
        if filename.startswith('mossy_larch'):
            ...
            maple_file_path = os.path.join(folder_path, filename.replace('mossy_larch', 'mossy_oak'))

            # Duplicate the file
            with open(pine_file_path, 'r') as pine_file:
                pine_content = pine_file.read()
            with open(maple_file_path, 'w') as maple_file:
                maple_file.write(pine_content.replace('mossy_larch', 'mossy_oak'))

            print(f'Duplicated and replaced: {filename} -> {filename.replace("mossy_larch", "mossy_oak")}')
