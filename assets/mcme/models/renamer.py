...
def duplicate_and_replace(folder_path):
    for filename in os.listdir(folder_path):
        if filename.startswith('oak'):
            ...
            maple_file_path = os.path.join(folder_path, filename.replace('oak', 'chestnut'))

            # Duplicate the file
            with open(spruce_file_path, 'r') as spruce_file:
                spruce_content = spruce_file.read()
            with open(maple_file_path, 'w') as maple_file:
                maple_file.write(spruce_content.replace('oak', 'chestnut'))

            print(f'Duplicated and replaced: {filename} -> {filename.replace("oak", "chestnut")}')
